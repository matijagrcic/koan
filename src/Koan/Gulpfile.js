...
gulp.task('build-styles', function() {
  return gulp.src('less/**/*.less')
    .pipe(less({
      paths: [npath.join(__dirname, 'less', 'includes')]
    }))
...
gulp.task('wwwroot', function() {
  return merge(
    gulp.src('jspm_packages/**/*').pipe(gulp.dest('wwwroot/jspm_packages')),
    gulp.src('config.js').pipe(gulp.dest('wwwroot'))
  );
});
...
gulp.task('build', function(callback) {
  return runSequence(
    'clean',
    ['build-system', 'build-html', 'build-styles', 'wwwroot'],
    callback
  );
});
